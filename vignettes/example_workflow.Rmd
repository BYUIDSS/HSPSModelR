...
train_x <- dhfr_reduced %>% slice( index) %>% select(-Y) %>% as_tibble()
test_x  <- dhfr_reduced %>% slice(-index) %>% select(-Y) %>% as_tibble()
train_y <- dhfr_reduced %>% dplyr::slice( index) %>% dplyr::select( Y) %>% as_vector() %>% unname()
test_y  <- dhfr_reduced %>% dplyr::slice(-index) %>% dplyr::select(Y) %>% as_vector() %>% unname()
```
...
```{r}
# This example dataset doesn't have an ID column
